        ...
        const resolvers = {
            Query: {
                ...
                tasksByStatus: (root: any, { status, userId }: { status: string; userId: string }) => {
                    ...
                    //     const tasks = await new ReadTasksByStatus(taskRepository).execute(readTasksByStatusDto!);
                    // console.log('SUCCESS ON FETCHING TASKS BY STATUS');
                    ...
                    // Lo anterior se puede reescrbir a continuacion en caso de que no sea asincrona async
                    const [error, readTasksByStatusDto] = ReadTasksByStatusDto.create({ userId, status });
            ...
            Mutation: {
                ...
                createTask: (_: any, { title, status, userId }: { title: string, status: string;  userId: string }) => {
                    ...
                    //     const task = await new CreateTask(taskRepository).execute(createTaskDto!);
                    //     return task;
                ...
                updateTask: (root: any, { taskId, title, status }: { taskId: string, title?: string, status?: string }) => {
                    ...
                    //     const task = await new UpdateTask(taskRepository).execute(updateTaskDto!);
                    //     return task;
                ...
                deleteTask: (root: any, { taskId }: { taskId: string }) => {
                    ...
                    //     const task = await new DeleteTask(taskRepository).execute(deleteTaskDto!);
                    //     return task;
